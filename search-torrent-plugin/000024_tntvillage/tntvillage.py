...
class TNTVillagePlugin(TorrentSearch.Plugin.Plugin):
   def _try_login(self):
      ...
      resp,content=c.request('http://forum.tntvillage.scambioetico.org/index.php?act=Login&CODE=00')
      data=urllib.urlencode({'UserName':username,'PassWord':password,'CookieDate':'1','Privacy':'1','submit':'Connettiti al Forum'})
      headers={'Content-type':'application/x-www-form-urlencoded', 'Cookie':resp['set-cookie']}
      resp,content=c.request("http://forum.tntvillage.scambioetico.org/index.php?act=Login&CODE=01","POST",data,headers)
